...
APK_DIR = os.path.abspath(os.path.join(os.path.dirname(__file__), '../../apk-builds/asm-app'))
# Debug print to verify resolved APK_DIR path
print(f"ASM APK_DIR resolved to: {APK_DIR}")

def get_apk_versions():
	files = os.listdir(APK_DIR)
	apk_pattern = re.compile(r"asm-app-(\d+)\.apk")
	versions = []
	for f in files:
...
@router.get("/download/{version}", summary="Download ASM app APK for specific version")
def download_asm_app_version(version: int):
	apk_name = f"asm-app-{version}.apk"
...
@router.get("/download/latest", summary="Download latest ASM app APK")
def download_latest_asm_app():
...
@router.get("/versions", summary="List available ASM app versions")
def list_asm_app_versions():
	versions = get_apk_versions()
